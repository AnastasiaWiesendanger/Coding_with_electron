(function () {
  ...
  // Variabile per il volume master delle frequenze dell'amico
  let friendVolumeMultiplier = 0.7; // Default 70%
  ...
  function updateAudioSynthesis() {
    ...
    // Applica il moltiplicatore del volume dello slider - ASSOCIAZIONI INVERTITE
    const doVolume = exponentialMapping(friendBattery, (0.4 / 3) * personalVolumeMultiplier, 3);
    const miVolume = exponentialMapping(friendRam, (0.4 / 3) * personalVolumeMultiplier, 3);
    const solVolume = exponentialMapping(friendCpuLoad, (0.4 / 3) * personalVolumeMultiplier, 3);
    
    // Note amico con curva esponenziale uguale (esponente 3) - ridotte del 30% (0.7) e controllate dal moltiplicatore friend
    // ASSOCIAZIONI INVERTITE
    const solFriendVolume = exponentialMapping(personalBattery, (0.4 / 3) * 0.7 * friendVolumeMultiplier, 3);
    const siFriendVolume = exponentialMapping(personalCpuLoad, (0.4 / 3) * 0.7 * friendVolumeMultiplier, 3);
    const reFriendVolume = exponentialMapping(personalRam, (0.4 / 3) * 0.7 * friendVolumeMultiplier, 3);
    ...
    // Aggiorna frequenze AM per note personali - ASSOCIAZIONI INVERTITE
    ampModOscillators.do.frequency.setTargetAtTime(mapToAMFrequency(friendBattery), now, 0.1);
    ampModOscillators.mi.frequency.setTargetAtTime(mapToAMFrequency(friendRam), now, 0.1);
    ampModOscillators.sol.frequency.setTargetAtTime(mapToAMFrequency(friendCpuLoad), now, 0.1);
    
    // Aggiorna frequenze AM per note amico - ASSOCIAZIONI INVERTITE
    ampModOscillators.solFriend.frequency.setTargetAtTime(mapToAMFrequency(personalBattery), now, 0.1);
    ampModOscillators.siFriend.frequency.setTargetAtTime(mapToAMFrequency(personalCpuLoad), now, 0.1);
    ampModOscillators.reFriend.frequency.setTargetAtTime(mapToAMFrequency(personalRam), now, 0.1);
  ...
  window.addEventListener("mouseUpdate", (e) => {
    ...
    // Mappa la posizione X del mouse (0-larghezza schermo) al range dello slider friend (0-100)
    ...
    // Aggiorna lo slider del volume delle note friend
    const friendSlider = document.getElementById('friend-volume-slider');
    const friendVolumeLabel = document.getElementById('friend-volume-label');
    if (friendSlider && friendVolumeLabel) {
      friendSlider.value = mouseXPercent;
      friendVolumeLabel.textContent = Math.round(mouseXPercent) + '%';
      
      // Aggiorna il moltiplicatore del volume friend (0-100% -> 0-1)
      friendVolumeMultiplier = mouseXPercent / 100;
    }
  });
  ...
  // Event listener per gli slider del volume (controllo manuale)
  document.addEventListener('DOMContentLoaded', () => {
    // Slider volume personale
    const personalSlider = document.getElementById('personal-volume-slider');
    const personalVolumeLabel = document.getElementById('volume-label');
    
    if (personalSlider && personalVolumeLabel) {
      personalSlider.addEventListener('input', (e) => {
        const sliderValue = parseFloat(e.target.value);
        personalVolumeLabel.textContent = Math.round(sliderValue) + '%';
        personalVolumeMultiplier = sliderValue / 100;
      });
    }
    
    // Slider volume friend
    const friendSlider = document.getElementById('friend-volume-slider');
    const friendVolumeLabel = document.getElementById('friend-volume-label');
    ...
    if (friendSlider && friendVolumeLabel) {
      friendSlider.addEventListener('input', (e) => {
        const sliderValue = parseFloat(e.target.value);
        friendVolumeLabel.textContent = Math.round(sliderValue) + '%';
        friendVolumeMultiplier = sliderValue / 100;
      });
    }
  });
      ...
      container.innerHTML = `
        <svg width="600" height="600" viewBox="0 0 600 600">
          ...
          <circle id="personal-battery" cx="300" cy="300" r="${personalBatteryRadius}" 
                  fill="rgba(255, 255, 0, 0.5)" stroke="rgba(255, 255, 0, 1)" 
          ...
          <circle id="friend-battery" cx="300" cy="300" r="${friendBatteryRadius}" 
                  fill="rgba(170, 0, 255, 0.5)" stroke="rgba(170, 0, 255, 1)" 
          ...
          <!-- Legenda -->
          <text id="personal-legend" x="300" y="550" text-anchor="middle" fill="white" font-size="12" opacity="0.8">
            Personal: B:${Math.round(personalBattery)}% C:${Math.round(personalCpuLoad)}% R:${Math.round(personalRam)}%
          </text>
          <text id="friend-legend" x="300" y="570" text-anchor="middle" fill="white" font-size="12" opacity="0.6">
